    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.row-datas.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('reward_status') ? 'is-invalid' : '' }}" name="reward_status" id="reward_status">
                    ...
                    @foreach(App\Models\RowData::REWARD_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('reward_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('used_by_phone') ? 'is-invalid' : '' }}" type="text" name="used_by_phone" id="used_by_phone" value="{{ old('used_by_phone', '') }}">
