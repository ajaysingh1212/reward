    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.winners.update", [$winner->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cupon_numbers') ? 'is-invalid' : '' }}" name="cupon_numbers[]" id="cupon_numbers" multiple required>
                    @foreach($cupon_numbers as $id => $cupon_number)
                        <option value="{{ $id }}" {{ (in_array($id, old('cupon_numbers', [])) || $winner->cupon_numbers->contains($id)) ? 'selected' : '' }}>{{ $cupon_number }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('full_name') ? 'is-invalid' : '' }}" type="text" name="full_name" id="full_name" value="{{ old('full_name', $winner->full_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $winner->phone_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $winner->email) }}">
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('upi') ? 'is-invalid' : '' }}" type="text" name="upi" id="upi" value="{{ old('upi', $winner->upi) }}" required>
                @if($errors->has('upi'))
                    <div class="invalid-feedback">
                        {{ $errors->first('upi') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('product_name') ? 'is-invalid' : '' }}" type="text" name="product_name" id="product_name" value="{{ old('product_name', $winner->product_name) }}">
